...
class dummy_handler(object):
    ...
    def on(self):
        print("ON");
        with open("brightness", "r") as f2:
            brightness = f2.read()
            for i in range(int(brightness)):
                b = i 
                for i in range(LED_COUNT):
                    led.setPixelColor(i, Color(int(b),int(b),int(b)))           
                led.show()
                time.sleep(0.01)
        return True
    ...
    def off(self):
        ...
        with open("brightness", "r") as f2:
            brightness = f2.read()
            for i in range(int(brightness), -1, -1):
                b = i 
                for i in range(LED_COUNT):
                    led.setPixelColor(i, Color(int(b),int(b),int(b)))               
                led.show()
